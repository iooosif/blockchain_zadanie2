...
// ============================================================
    ...
    function addLiquidity(uint max_exchange_rate, uint min_exchange_rate) external payable {
        ...
        if (total_shares == 0) {
            ...
            provider_share = (msg.value * total_shares) / eth_reserves;
            total_shares += provider_share;
        }
        ...
        payable
    {
        ...
        require(amountETH <= (lps[msg.sender] * eth_reserves) / total_shares, "Not enough liquidity provided by sender");

        // Check the current exchange rate (PRMT per 1 ETH)
        uint current_exchange_rate = (token_reserves * 1000) / eth_reserves; // Multiply by 1000 for precision
        require(current_exchange_rate <= max_exchange_rate, "Slippage too high: exchange rate above maximum");
        require(current_exchange_rate >= min_exchange_rate, "Slippage too low: exchange rate below minimum");
        ...
        uint share_correction = (amountETH * total_shares) / eth_reserves;
        total_shares -= share_correction;
        ...
        payable
    {
        ...
        // Check the current exchange rate (ETH per 1 PRMT)
        uint current_exchange_rate = (amountETH * 1000) / amountTokens; // Multiply by 1000 for precision
        ...
        payable
    {
        ...
        // Check the current exchange rate (PRMT per 1 ETH)
        uint current_exchange_rate = (amountTokens * 1000) / msg.value; // Multiply by 1000 for precision
        require(current_exchange_rate >= min_exchange_rate, "Slippage too high: exchange rate below minimum");
